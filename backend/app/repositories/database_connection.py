...
class DatabaseConnectionRepo(BaseRepo):
    ...
    def search(
        ...
        """
        statement = select(DatabaseConnection).where(DatabaseConnection.deleted_at.is_(None))
        ...
        if database_type:
            statement = statement.where(DatabaseConnection.database_type == database_type)
