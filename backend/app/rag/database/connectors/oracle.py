...
from app.parameters.database_connection import OracleParameters
...
class OracleConnector(BaseConnector):
    ...
    def __init__(self, connection: DatabaseConnection):
        ...
        self.parameters: Optional[OracleParameters] = None
    ...
    def _init_parameters(self) -> None:
        ...
        config = decrypt_dict_values(
            ...
            OracleParameters.SENSITIVE_FIELDS
        )
        
        # 创建参数对象
        self.parameters = OracleParameters.from_dict(config)
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            # 更新连接状态
            logger.info(f"Successfully connected to Oracle database: {self.parameters.database}")
            return True
    ...
    def test_connection(self) -> ConnectionTestResult:
        ...
        try:
            ...
            with self.get_connection() as conn:
                result = conn.execute(text("SELECT BANNER FROM V$VERSION WHERE BANNER LIKE 'Oracle%'")).fetchone()
                version = result[0] if result else "Unknown"
                
                # 获取表数量
                tables_result = conn.execute(text(
                    f"SELECT COUNT(*) as table_count FROM ALL_TABLES WHERE OWNER = '{self.parameters.user.upper()}'"
                )).fetchone()
            ...
            return ConnectionTestResult(
                success=True,
                message=f"Successfully connected to {version}",
                details={
                    "version": version,
    ...
    def get_metadata(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 获取所有表信息
            tables_metadata = {}
            schema = self.parameters.user.upper()
            ...
            for table_name in inspector.get_table_names(schema=schema):
                ...
                tables_metadata[f"{schema}.{table_name}"] = {
                    "schema": schema,
                    "name": table_name,
                    "columns": columns,
    ...
    def execute_query(self, query: str, params: Optional[Dict[str, Any]] = None) -> Tuple[List[Dict[str, Any]], Optional[str]]:
        ...
        if self.connection_config.read_only:
            ...
            if not (query_upper.startswith("SELECT") or query_upper.startswith("WITH") or 
                   query_upper.startswith("DECLARE") or query_upper.startswith("BEGIN")):
        ...
        try:
            # 使用cx_Oracle直接执行查询以获取字典格式结果
            conn_string = None
            if self.parameters.service_name:
                conn_string = f"{self.parameters.host}:{self.parameters.port}/{self.parameters.service_name}"
            elif self.parameters.sid:
                conn_string = f"{self.parameters.host}:{self.parameters.port}/{self.parameters.sid}"
            else:
                conn_string = f"{self.parameters.host}:{self.parameters.port}/{self.parameters.database}"
