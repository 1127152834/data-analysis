...
class SQLServerConnector(BaseConnector):
    ...
    def _init_parameters(self) -> None:
        ...
        config = decrypt_dict_values(
            ...
            SQLServerParameters.SENSITIVE_FIELDS
        )
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            # 更新连接状态
            logger.info(f"Successfully connected to SQL Server database: {self.parameters.database}")
            return True
    ...
    def execute_query(self, query: str, params: Optional[Dict[str, Any]] = None) -> Tuple[List[Dict[str, Any]], Optional[str]]:
        ...
        if self.connection_config.read_only:
            ...
            if not (query_upper.startswith("SELECT") or query_upper.startswith("WITH") or 
                   query_upper.startswith("DECLARE") or query_upper.startswith("EXEC SP_") or
                   query_upper.startswith("EXECUTE SP_")):
