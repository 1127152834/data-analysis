...
def list_all_chat_engines(session: SessionDep, user: CurrentSuperuserDep):
    ...
    engines = session.exec(select(ChatEngine).where(ChatEngine.deleted_at == None)).all()
...
def list_linked_engines_for_kb(session: SessionDep, user: CurrentSuperuserDep, kb_id: int):
