                ...
                ? current.map((option, index) => (
                  option ? (
                    <div key={option.id} className="w-full block border-t first-of-type:border-t-0 py-2">
                      ...
                      <span>
                        {(option.documents_total ?? 0) || <><AlertTriangleIcon className="text-warning inline-flex size-3 mr-0.5" /> 无</>} 文档
                      </span>
                        <DotIcon className="size-4" />
                        <span className="text-xs text-muted-foreground">
                        {(option.data_sources_total ?? 0) || <><AlertTriangleIcon className="inline-flex size-3 mr-0.5" /> 无</>} 数据源
                      </span>
                      </div>
                    </div>
                  ) : <span key={value?.[index]}>未知知识库 {value?.[index]}</span>
                )) : <span className="pt-1 text-muted-foreground">选择知识库</span>
          }
        </PopoverPrimitive.Trigger>
      </div>
