  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-muted-foreground text-xs space-y-2">
        <div>数据集: <Link className="text-foreground underline" href={`/evaluation/datasets/${task.dataset_id}`}>{task.dataset_id}</Link></div>
        <div>创建时间: <DateFormat date={task.created_at} /></div>
        <div>更新时间: <DateFormat date={task.updated_at} /></div>
        <div>用户ID: {task.user_id}</div>
        {canCancel && <div>
  ...
  return (
    <Card className="flex flex-col">
      <CardHeader className="items-center pb-0">
        <CardTitle className="text-lg font-normal">评估项目</CardTitle>
      </CardHeader>
...
export function RagasMetricsChart ({ summary }: { summary: Pick<EvaluationTaskSummaryType, keyof EvaluationTaskSummaryType & (`${string}_correctness` | `${string}_similarity`)> }) {
  const chartData = useMemo(() => {
    return [
      { metrics: 'min', label: '最小值', semantic_similarity: summary.min_semantic_similarity, factual_correctness: summary.min_factual_correctness },
      { metrics: 'max', label: '最大值', semantic_similarity: summary.max_semantic_similarity, factual_correctness: summary.max_factual_correctness },
      { metrics: 'avg', label: '平均值', semantic_similarity: summary.avg_semantic_similarity, factual_correctness: summary.avg_factual_correctness },
      { metrics: 'std', label: '标准差', semantic_similarity: summary.std_semantic_similarity, factual_correctness: summary.std_factual_correctness },
    ];
  }, [summary]);
  ...
  return (
    <Card className="flex flex-col justify-between h-full">
      <CardHeader className="items-center pb-0">
        <CardTitle className="text-lg font-normal">评估指标</CardTitle>
      </CardHeader>
          ...
          <ChartContainer className="max-h-[192px] mx-auto" config={chartConfig}>
            <BarChart accessibilityLayer data={chartData}>
              ...
              <XAxis
                ...
                axisLine={false}
                tickFormatter={(value) => {
                  const item = chartData.find(d => d.metrics === value);
                  return item?.label || value.slice(0, 3);
                }}
              />
              <ChartTooltip
                cursor={false}
