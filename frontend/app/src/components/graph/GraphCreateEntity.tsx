  ...
  return (
    <Form {...form}>
      <form className={cn('space-y-4', className)} onSubmit={handleSubmit}>
        <h2 className="font-bold text-xl">创建摘要实体</h2>
        <FormField
          name="name"
          render={({ field }) => (
            <FormItem>
        ...
        <FormItem>
          ...
          <FormControl>
            <div className="relative w-full h-32 border" ref={containerRef}>
              <Suspense fallback={<Loader loading>正在初始化JSON编辑器...</Loader>}>
              ...
              {entities.map(entity => (
                <Tooltip key={entity.id}>
                  ...
                  <TooltipContent className="space-y-2 w-[360px]">
                    ...
                    <Button variant="secondary" className="w-full mt-4" size="sm" onClick={() => onClearSelection(Number(entity.id))}>从实体中移除</Button>
                  </TooltipContent>
                </Tooltip>
              ))}
            </div>
