...
export function GraphEditor ({ knowledgeBaseId }: { knowledgeBaseId: number }) {
  const [query, setQuery] = useSearchParam('query', 'sample-question:什么是深度研究');
...
function SubgraphSelector ({ knowledgeBaseId, query, onQueryChange }: { knowledgeBaseId: number, query: string | null, onQueryChange: (query: string) => void }) {
  const [initialType = 'sample-question', initialInput = '什么是深度研究'] = parseQuery(query) ?? [];

  const [type, setType] = useState<string>(initialType);
  const [input, setInput] = useState<string>(initialInput);

  useEffect(() => {
    const [type = 'sample-question', input = '什么是深度研究'] = parseQuery(query) ?? [];
    setType(type);
    setInput(input);
  }, [query]);
  ...
  return (
    <div className="flex gap-2">
      <Select value={type} onValueChange={type => {
        ...
        <SelectContent>
          <SelectItem value="sample-question">示例问题</SelectItem>
          <SelectItem value="entity">实体 ID</SelectItem>
          <SelectItem value="message-subgraph">消息子图</SelectItem>
          <SelectItem value="trace" disabled>Langfuse Trace ID (UUID)</SelectItem>
          <SelectItem value="document" disabled>文档 URI</SelectItem>
        </SelectContent>
      </Select>
      <Input
        className="flex-1"
