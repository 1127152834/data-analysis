    ...
    >
      ...
      <div className="my-2 px-4 flex items-center flex-wrap justify-between">
        <div className="text-xs text-muted-foreground">
          由 <a className="underline font-bold" href="https://www.pingcap.com/tidb-cloud-serverless/?utm_source=tidb.ai&utm_medium=community" target="_blank">TiDB Serverless</a> 提供支持
        </div>
...
const defaultMessages: Record<string, string> = {
  'creating': '准备验证消息...',
  'citation_unavailable': '引用不可用',
  'unreachable': '无法检查',
  'unverified': '未验证',
  'conflict': '内容冲突',
  'error': '未知错误',
  [VerifyStatus.CREATED]: '准备验证消息...',
  [VerifyStatus.EXTRACTING]: '提取SQL...',
  [VerifyStatus.VALIDATING]: '验证结果...',
  [VerifyStatus.SUCCESS]: '验证成功',
  [VerifyStatus.FAILED]: '验证失败',
  [VerifyStatus.SKIPPED]: '未验证'
};
...
function MessageVerifyHeader ({ creating, error, result }: { creating?: boolean, error: unknown, result: MessageVerifyResponse | undefined }) {
  ...
  if (creating) {
    ...
    message = result?.message ?? (result ? defaultMessages[result.status] : undefined) ?? '未知验证状态。';
  }
